...
salmon.abundance <- readxl::read_xlsx(here::here("modelfiles","Salmon_timeline_Losee.xlsx"), sheet = 1) %>% 
 # dplyr::rename("Salmon" = `Salmon Sum`) %>% 
  ...
  pivot_longer(cols=Chinook:Sockeye,names_to = "salmon_group",values_to = "biomass_mt")
...
salmon.effect <- c(rep("Negative", 8), rep("Positive",1), rep("Positive", 8), rep("Negative",1))
...
#create table with expected effects by scenario
sc.name <- rep(c("Cumulative effects"),2)
scenario.var <- c("+20%", "-20%")
salmon.effect <- c("Negative impacts on salmon", "Positive impacts on salmon")
scenario.effect <- dplyr::tibble(scenario_name=sc.name, scenario_var = scenario.var, salmon_effect = salmon.effect)
...
plot_ensemble_cum_survival_scenarios(ensemblenumbersagescenarios, salmongroups, base.cum.survival, salmonbybasin, scenario.effect)
