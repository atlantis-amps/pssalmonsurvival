...
plot_predation <- function(ensemblepredationcum, salmoneffect){
...
# HEATMAP, not used because it can't incorporate variation in the model ensemble
# pred.plot.salmon %>%
# ggplot(aes(salmon_effect, longname)) +
#   geom_tile(aes(fill = log(excess_mort))) +
# scale_fill_gradient(low = "lightyellow", high = "firebrick4") +
...
pred.boxplot <- pred.plot.salmon %>%
  ...
  ggplot2::labs(title = "Cumulative scenarios", y = "% change in mortality relative to base scenario", x = "Functional group", face = "bold") +
  ggthemes::theme_base() +
  ggplot2::theme(legend.position="bottom") +
  ggplot2::theme(axis.text.x=ggplot2::element_text(angle=90, vjust=0.5, hjust=0.95))

# pred.plot.salmon %>%
#   dplyr::filter(scenario_name=="Top-down") %>%
#   ggplot2::ggplot(ggplot2::aes(y = excess_mort, x = salmon_effect, fill = basin)) +
#   ggplot2::geom_boxplot(width = 0.1) +
# #  ggplot2::geom_violin(width=1.4) +
#   ggplot2::geom_hline(yintercept = 1) +
#   ggplot2::facet_wrap( ~ longname, scales = "free_y") +
#   ggplot2::scale_fill_manual(values = col.fill, name = "Basin of origin") +
#   ggplot2::labs(title = "Cumulative scenarios", y = "% change in mortality relative to base scenario", x = "Functional group", face = "bold") +
#   ggthemes::theme_base() +
#   ggplot2::theme(legend.position="bottom") +
...
#
# pred.plot.salmon %>%
#   dplyr::filter(scenario_name=="Bottom-up & Top-down") %>%
#   dplyr::filter(salmon_effect=="Positive impacts on salmon") %>%
#   ggplot2::ggplot(ggplot2::aes(x = excess_mort, fill = basin)) +
