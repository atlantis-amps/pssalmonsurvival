 ...
 # lapply(file.names, get_shapefile, shapefile.name) # get shapefile, download as raw
  ...
  model.map <-  ggplot2::ggplot(data = coastline.shape) +
    ...
    ggplot2::geom_text(data = cities.shape, ggplot2::aes(x = LONG, y = LAT, label = Name),
                       size = 3, col = "black", fontface = "plain", nudge_y = -0.04, nudge_x = 0.023) +
    ...
    ggplot2::ylab("Lat")+
    ggplot2::theme_bw() +
    ggplot2::theme(axis.text = ggplot2::element_text(size = 9), axis.title = ggplot2::element_text(size = 11), axis.text.y = ggplot2::element_text(angle = 90))
