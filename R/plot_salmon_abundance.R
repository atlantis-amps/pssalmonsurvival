...
pal.col <- wesanderson::wes_palette("Cavalcanti1",5)
  ...
  salmon.plot <- salmon.abundance %>%
    ggplot2::ggplot(ggplot2::aes(x=Year, y=biomass_mt, group=`salmon_group`)) +
    ggplot2::geom_line(ggplot2::aes(color = salmon_group, group = salmon_group, linetype = salmon_group %in% c("Pink")), size = 0.7) +
    ggplot2::scale_linetype_manual(values = c("TRUE" = "twodash", "FALSE" = "solid")) +
    ggplot2::scale_color_manual("Salmon group", values=pal.col) +
    ...
    ggplot2::scale_y_continuous(name ="Biomass (metric tons)") +
